...
ProgressBar& ProgressBar::operator=(const ProgressBar& rhs) {
  _ledController = rhs._ledController;
  _max = rhs._max;
  _current = rhs._current;
  _r = rhs._r;
  _g = rhs._g;
  _b = rhs._b;
  _r_before = rhs._r_before;
  _g_before = rhs._g_before;
  _b_before = rhs._b_before;
  _r_after = rhs._r_after;
  _g_after = rhs._g_after;
  _b_after = rhs._b_after;
  _marks_offset = rhs._marks_offset;
  _marks_step = rhs._marks_step;
  return *this; 
}
...
void ProgressBar::update() {
  ...
  for (float mark = _marks_offset ; mark < _max ; mark += _marks_step) {
  ...
  for (i = 0 ; i < _ledController->numPixels() ; i++) {
    //The following code may look the same, bu on is for numbering the pixels top to bottom, and the other for bottom to top.
    //_ledController->setPixelColor((_ledController->numPixels()-1)-i, cols[i]);
...
void ProgressBar::setMarks(float offset, float step) {
  if (step <= 0.0) return;
  _marks_offset = offset;
  _marks_step = step;
}
